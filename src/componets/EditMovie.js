...
const EditMovie = () => {
    ...
    const handleCheck = (event, position) => {
        ...
        let temArr = movie.genres
        temArr[position].checked = !temArr[position].checked

        let temIDs = movie.genres_array;

        if (!event.target.checked) {
            temIDs.splice(temIDs.indexOf(event.target.value));
        } else {
            temIDs.push(parseInt(event.target.value, 10))
        }
